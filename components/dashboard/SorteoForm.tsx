...
export function SorteoForm({ sorteo, userId }: SorteoFormProps) {
  ...
  const { register, handleSubmit, watch, setValue, control, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      premios: [{ nombre: '', descripcion: '', valor_estimado: 0, imagen_url: '' }],
  ...
  const uploadImagen = async (file: File, index: number) => {
    ...
    const { data, error } = await supabase.storage.from('sorteos').upload(path, file, { upsert: true })
    if (error) { toast.error('Error al subir imagen'); return }
    const { data: { publicUrl } } = supabase.storage.from('sorteos').getPublicUrl(data.path)
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (isEdit) { await sb.from('cuentas_deposito').delete().eq('usuario_id', userId) }
      ...
      toast.success('Sorteo enviado a revisión. El administrador lo aprobará pronto.')
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-2xl">
      <div className="bg-brand-card border border-brand-border rounded-2xl p-6 space-y-4">
        <h2 className="font-ui font-semibold text-white">Información del sorteo</h2>
        ...
        <div className="space-y-1.5">
          ...
          <textarea className="flex w-full rounded-lg border border-brand-border bg-brand-card px-3 py-2 text-sm text-white placeholder:text-brand-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary min-h-[80px] resize-none" placeholder="Descripción corta del sorteo..." {...register('descripcion')} />
        </div>
      ...
      <div className="bg-brand-card border border-brand-border rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-ui font-semibold text-white">Premios (máx. 3)</h2>
        ...
        {premioFields.map((field, i) => (
          <div key={field.id} className="p-4 rounded-xl border border-brand-border/60 bg-brand-border/10 space-y-3">
            ...
            <Input placeholder="Descripción breve" {...register(`premios.${i}.descripcion`)} />
      ...
      <div className="flex gap-3">
        ...
        <Button type="submit" className="flex-1" disabled={guardando}>
          {guardando ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Guardando...</> : isEdit ? 'Actualizar Sorteo' : 'Enviar a Revisión'}
        </Button>
      </div>
    </form>
  )
